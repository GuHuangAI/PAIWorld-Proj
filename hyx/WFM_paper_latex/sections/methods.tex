...
We present \methodname{}, a framework for injecting 3D consistency into flow-matching world foundation models for multi-view robotic manipulation. As argued in \secref{sec:introduction}, multi-view 3D consistency rests on two conditions: inter-view communication and a 3D geometric prior. Our approach augments a DiT-based flow matching backbone with three components that satisfy both conditions jointly (\figref{fig:pipeline}): Geometric Rotary Position Embedding (\secref{sec:camera-pe}), Geometry-Aware Cross-View Attention (\secref{sec:cross-view-attn}), and Latent 3D-REPA (\secref{sec:3d-repa}). We first formalize the problem setting (\secref{sec:problem}), then describe each component and analyze why both conditions must hold simultaneously (\secref{sec:synergy}).
...
\end{equation}
where $T$ is the prediction horizon. The generated multi-view video must satisfy a \emph{multi-view 3D consistency} constraint: for any 3D point $\mathbf{X} \in \R^3$ visible in views $v_i$ and $v_j$ at time $t$, its projected pixel locations $\mathbf{x}_t^{v_i}$ and $\mathbf{x}_t^{v_j}$ must satisfy the epipolar constraint:
\begin{equation}
\label{eq:epipolar}
(\mathbf{x}_t^{v_j})^\top \mathbf{F}_{v_i \to v_j} \, \mathbf{x}_t^{v_i} = 0,
\end{equation}
where $\mathbf{F}_{v_i \to v_j}$ is the fundamental matrix derived from the relative camera pose between views $v_i$ and $v_j$. More broadly, the generated multi-view video should admit a coherent 3D reconstruction at every time step, i.e., there exists a consistent 3D scene $\mathcal{S}_t$ such that all views $\{I_t^v\}_{v=1}^V$ can be obtained by rendering $\mathcal{S}_t$ from their respective camera poses.

\subsection{Flow Matching DiT for Video Generation}

We adopt a Diffusion Transformer (DiT)~\cite{peebles2023dit} trained with a flow matching objective~\cite{lipman2023flow,esser2024sd3} operating in the latent space of a pretrained video VAE. Given an input video, the VAE encoder produces a latent representation $\feat_0 \in \R^{T \times H \times W \times C}$. The model learns a velocity field $v_\theta(\feat_s, s)$ that transports samples from a noise distribution toward the data distribution along a linear interpolation path:
\begin{equation}
...
At selected layers (the first 3 and last 3 blocks of the DiT), we insert a dedicated \emph{Cross-View Attention} sub-block. For each temporal frame $t$ (distinct from the flow timestep $s$), let $\{\mathbf{Z}_t^v\}_{v=1}^{V}$ denote the feature maps from all views, where $\mathbf{Z}_t^v \in \R^{(H \cdot W) \times D}$. The Cross-View Attention explicitly incorporates Geo-RoPE into the query and key projections:
\begin{equation}
\label{eq:cross-view-attn}
\begin{aligned}
\tilde{\mathbf{Q}}_t^v &= \text{GeoRoPE}(\mathbf{W}_Q \mathbf{Z}_t^v) \\
\tilde{\mathbf{K}}_t &= \text{GeoRoPE}(\mathbf{W}_K [\mathbf{Z}_t^1; \ldots; \mathbf{Z}_t^V]) \\
\hat{\mathbf{Z}}_t^v &= \mathbf{Z}_t^v + \text{gate} \cdot \text{softmax}\!\left(\frac{\tilde{\mathbf{Q}}_t^v {\tilde{\mathbf{K}}_t}^\top}{\sqrt{d}}\right) \mathbf{W}_V [\mathbf{Z}_t^1; \ldots; \mathbf{Z}_t^V],
\end{aligned}
\end{equation}
where GeoRoPE applies the split ray-pose rotary encoding from \eqnref{eq:geo-rope} to the projected queries and keys. This ensures that geometrically corresponding tokens across views---those observing the same 3D point---naturally receive higher attention weights due to their aligned rotary positions. The gate is initialized to zero via AdaLN-Zero, preserving the pretrained single-view model at initialization.
...
Cross-View Attention provides the information pathway for viewpoints to exchange geometric and semantic information, without which each view evolves independently with no mechanism to enforce cross-view coherence. Communication alone, however, does not guarantee 3D consistency: lacking an explicit geometric objective, the attention can converge to trivial shortcuts such as copying textures across views. This motivates the geometric learning signal introduced next.
